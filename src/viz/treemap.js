 ...
 *
 * Input: items = [{ name, value, color?, children?: [{name, value}] }, ...]
 * Output: inline SVG string with rounded-corner tiles, category label, %.
 *
 * Algorithm: Bruls/Huijbregts/van Wijk "Squarified Treemaps" (2000).
 * Lightweight rewrite — no external deps.
 */
...
function squarify(items, total, rect) {
  ...
  while (remaining.length > 0) {
    ...
    while (remaining.length > 0) {
      ...
      const candidateSum = rowSum + next.value;
      const worstCurrent = row.length === 0 ? Infinity : worstRatio(row, rowSum, shortSide, box.w * box.h, totalLeft);
      const worstNext = worstRatio(candidate, candidateSum, shortSide, box.w * box.h, totalLeft);
    ...
    // Layout row along the short side.
...
function worstRatio(row, sum, shortSide, area, totalLeft) {
